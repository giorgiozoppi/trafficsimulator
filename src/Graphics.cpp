...
void Graphics::drawTrafficObjects() {
  ...
  for (auto it : _trafficObjects) {
    ...
    if (it->getType() == ObjectType::objectIntersection) {
      ...
      cv::Scalar trafficLightColor = intersection->trafficLightIsGreen()
                                         ? cv::Scalar(0, 255, 0)
                                         : cv::Scalar(0, 0, 255);
      cv::circle(_images.at(0), cv::Point2d(posx, posy), 25, trafficLightColor,
                 -1);
  ...
  auto img = _images.at(2);
  cv::Mat outImg;
  cv::resize(img, outImg, cv::Size(), 0.50, 0.50);
  cv::imshow(_windowName, outImg);
  cv::waitKey(33);
}
