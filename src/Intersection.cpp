...
void WaitingVehicles::permitEntryToFirstInQueue()
{
    ...
    auto vehicle = _vehicles.front();
    auto promise = std::move(_promises.front());
    promise.set_value();
...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    std::promise<void> waiting_promise;
    auto waiting_grant = waiting_promise.get_future();
    _waitingVehicles.pushBack(vehicle, std::move(waiting_promise));
    waiting_grant.get();
